...
class HomestretchApiClientError(Exception):
    """Base exception to indicate a general API error."""


class HomestretchApiClientCommunicationError(
    HomestretchApiClientError,
):
    """Exception to indicate a communication error with the API."""


class HomestretchApiClientAuthenticationError(
    HomestretchApiClientError,
):
...
def _verify_response_or_raise(response: aiohttp.ClientResponse) -> None:
    ...
    Raises:
        HomestretchApiClientAuthenticationError: For 401/403 errors.
        aiohttp.ClientResponseError: For other HTTP errors.

    """
    if response.status in (401, 403):
        msg = "Invalid credentials"
        raise HomestretchApiClientAuthenticationError(
            msg,
        )
    response.raise_for_status()


class HomestretchApiClient:
    """
    ...
    async def async_get_data(self) -> Any:
        ...
        Raises:
            HomestretchApiClientAuthenticationError: If authentication fails.
    ...
    async def _api_wrapper(
        ...
        try:
            ...
            raise HomestretchApiClientCommunicationError(
                msg,
            ) from exception
            ...
            raise HomestretchApiClientError(
                msg,
            ) from exception
