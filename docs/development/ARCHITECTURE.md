...
This document describes the technical architecture of the Homestretch custom component for Home Assistant.

## Directory Structure

```text
custom_components/homestretch/
...
│   ├── __init__.py          # Exports HomestretchDataUpdateCoordinator
...
│   ├── __init__.py          # Exports HomestretchEntity
...
- `base.py` - Main coordinator class (`HomestretchDataUpdateCoordinator`)
...
**Key class:** `HomestretchDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `HomestretchApiClient`
...
- `HomestretchConfigFlowHandler` (main flow)
- `HomestretchOptionsFlow` (options)
...
**Key class:** `HomestretchEntity` (in `entity/base.py`)
...
2. `HomestretchEntity` for common functionality
...
1. Create directory: `custom_components/homestretch/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `HomestretchEntity`
