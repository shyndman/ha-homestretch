...
This document describes all configuration options and settings available in the Homestretch custom integration.
...
```yaml
service: homestretch.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: homestretch.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
- [GitHub Issues](https://github.com/shyndman/ha-homestretch/issues) - Report problems
